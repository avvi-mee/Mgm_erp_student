...
const sem2Courses=[
["APS21BSL102","ENGINEERING PHYSICS","40.00","16.00","56.00","3.00","B+","5.99"],
["APS21BSL103","LINEAR ALGEBRA AND DIFFERENTIAL EQUATIONS","25.00","16.00","41.00","4.00","C","4.49"],
["APS21BSP101","ENGINEERING PHYSICS LAB","24.00","11.00","35.00","1.00","A+","7.99"],
["APS21ESL102","ENGINEERING GRAPHICS","28.00","16.00","44.00","2.00","C","4.49"],
["APS21ESL104","BUILDING PROGRAMMING LOGIC IN C","14.00","9.00","23.00","1.00","C+","4.99"],
["APS21ESP102","ENGINEERING GRAPHICS STUDIO","20.00","15.00","35.00","2.00","A+","7.99"],
["APS21ESP103","RECENT TRENDS IN INTEGRATED TECHNOLOGY","23.00","16.00","39.00","1.00","A+","7.99"],
["APS21ESP105","BUILDING PROGRAMMING LOGIC IN C LAB","23.00","15.00","38.00","1.00","A+","7.99"],
["APS21IKL101","INDIAN KNOWLEDGE SYSTEM","31.00","16.00","47.00","2.00","C+","4.99"],
["APS21WSP101","ENGINEERING EXPLORATION","28.00","20.00","48.00","2.00","C+","4.99"],
["MGM05CCP101","CULTURAL ACTIVITIES","32.00","0.00","32.00","2.00","A","6.99"]
];
...
function renderResult(){
 ...
 <div class="filters">
  <div class="filter"><label>Select Exam:</label><select id="examSelect"><option value="">Select</option><option value="ET WINTER (REG) 2025">ET WINTER (REG) 2025</option><option value="ET SUMMER (REG) 2026">ET SUMMER (REG) 2026</option></select></div>
  <div class="filter"><label>Select Term:</label><select id="termSelect"><option value="">Select</option><option value="I">I</option><option value="II">II</option></select></div>
 </div>
...
function renderResultTable(exam,term){
 const isSem2=term==="II";
 const rows=isSem2?sem2Courses:courses;
 const sgpa=isSem2?"5.89":"5.63";
 const percentage=isSem2?"60.86 %":"59.23 %";
 document.getElementById("resultArea").innerHTML=`
 <div class="result-wrap ${isSem2?"sem2-result":""}"><div class="tab">Term (${term}) Result</div>
 ...
 <tbody>${rows.map((c,i)=>`<tr><td>${i+1}</td><td><b>${c[0]}</b><br>Int:${c[2].replace(".00","")} Ext:${c[3].replace(".00","")}</td><td class="course">${c[1]}</td><td>${c[2]}</td><td>${c[3]}</td><td>${c[4]}</td><td>${c[5]}</td><td>${c[6]}</td><td>${c[7]}</td></tr>`).join("")}</tbody></table>
 <div class="summary ${isSem2?"sem2-summary":""}"><div>SGPA: ${sgpa}</div><div>CGPA: ${isSem2?"6.78":"--"}</div><div>Percentage: ${percentage}</div><div class="pass">Result : Pass</div></div>
 </div>`;
}
function renderSimple(type){
 ...
 if(type==="attendance"){
   ...
   <div class="attendance-page">
     ...
     <div class="attendance-wrap">
       ...
       <tbody>
       ${[
       ["APS21ESL104","BUILDING PROGRAMMING LOGIC IN C","40","Faculty Name","19","16","0","5","35","54.30"],
 ...
 if(type==="marks"){
   document.getElementById("main").innerHTML=`
   <div class="simple-page">
    <div class="breadcrumb">ACADEMIC SCHEDULES » MARKS</div>
    <div class="card" style="padding:20px">
      <div class="term-bar"><label>Term</label><select><option>Sem 2</option><option>Sem 3</option></select></div>
      <div class="page-tabs">
       <button class="page-tab" onclick="renderSimple('courses')">Courses</button>
       <button class="page-tab" onclick="renderSimple('timetable')">Schedule</button>
       <button class="page-tab" onclick="renderSimple('attendance')">Attendance</button>
       <button class="page-tab active">Marks</button>
       <button class="page-tab" onclick="renderSimple('assessments')">Assessment</button>
       <button class="page-tab" onclick="modal('Shared Contents','Shared course contents opened successfully.')">Shared Contents</button>
      </div>
      <table class="simple-table"><tr><th>Course Code</th><th>Course Name</th><th>Marks</th></tr>
      <tr><td>APS21ESL104</td><td>BUILDING PROGRAMMING LOGIC IN C</td><td><strong>10</strong></td></tr>
      </table>
    </div>
   </div>`;
   return;
